...
def generate_all(config: dict, paths: Paths, force: bool = False) -> Path:
    ...
    tokenizer = AutoTokenizer.from_pretrained(paths.models / "qwen2_5_3b_instruct", local_files_only=True); tokenizer.pad_token = tokenizer.eos_token
    base = AutoModelForCausalLM.from_pretrained(paths.models / "qwen2_5_3b_instruct", torch_dtype=torch.bfloat16, local_files_only=True, attn_implementation="sdpa").cuda().eval()
    arms = [("qwen", base, False), ("qwen_rag", base, True)]
    for name, model, rag in arms:
        output_path = stage / f"{name}.jsonl"
        if output_path.exists() and not force:
            continue
        partial_path = output_path.with_suffix(".partial.jsonl")
        outputs = _generate(model, tokenizer, rows, contexts, rag, config, partial_path)
        output_path.write_text("\n".join(json.dumps(item, ensure_ascii=False) for item in outputs) + "\n", encoding="utf-8")
        partial_path.unlink(missing_ok=True)
    del base; torch.cuda.empty_cache()
    base = AutoModelForCausalLM.from_pretrained(paths.models / "qwen2_5_3b_instruct", torch_dtype=torch.bfloat16, local_files_only=True, attn_implementation="sdpa").cuda().eval()
    adapted = PeftModel.from_pretrained(base, paths.models / "pissa_qwen2_5_3b_r32_lora_best", local_files_only=True).eval()
    # The converted LoRA adapter is inference-only here. Merge once to remove
    # per-projection PEFT dispatch overhead during the 1,000-example ablation.
    ...
    for name, rag in (("qwen_pissa", False), ("qwen_pissa_rag", True)):
        ...
        output_path.write_text("\n".join(json.dumps(item, ensure_ascii=False) for item in outputs) + "\n", encoding="utf-8")
        partial_path.unlink(missing_ok=True)
    stage_manifest(stage, config=config, inputs={"evaluation_rows": len(rows), "retriever": "hybrid_reranker"}, outputs={"arms": 4, "records_per_arm": len(rows), "backend": "transformers_sdpa_bf16"})
    return stage
